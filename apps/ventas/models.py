...
class MetodoPago(models.Model):
    """
    Mapea a public.metodo_pago existente:
    - id_metodo_pago (PK, serial)
    - tipo (varchar)
    - categoria (varchar)
    - requiere_pasarela (bool)
    - codigo_pasarela (varchar, nullable)
...
class PaymentTransaction(models.Model):
    """
    Mapea a public.transaccion_pago existente:
    - id_transaccion (PK, serial)
    - id_venta (int, FK)
    - id_metodo_pago (int, FK)
    - monto (numeric)
    - fecha_transaccion (timestamp, default now())
    - estado_transaccion (varchar, default 'PENDIENTE')
    - referencia_externa (varchar, nullable)
    - descripcion (text, nullable)
    - codigo_error (varchar, nullable)
    - procesado_por (int, nullable)
...
class Venta(models.Model):
    """
    Mapea a la tabla public.venta para validaciones de existencia.
    """
